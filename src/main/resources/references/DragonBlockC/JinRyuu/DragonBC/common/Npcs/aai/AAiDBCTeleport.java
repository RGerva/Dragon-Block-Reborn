...
public class AAiDBCTeleport extends AAi {
   ...
   public void update() {
      ...
      if (entity.func_70089_S() && !entity.world.field_72995_K && (EnemyTeleportOutOfLock || !entity.isLocked()) && !entity.chargingKiAttack) {
         ++this.timer;
         List list = entity.world.func_72872_a(EntityPlayer.class, entity.boundingBox.func_72314_b(16.0D, 16.0D, 16.0D));
         if (!list.isEmpty()) {
            for(int i = 0; i < list.size(); ++i) {
               ...
               if (this.timer >= this.rate) {
                  this.rate = this.generateRate();
                  this.timer = 0;
                  entity.func_70634_a(target.posX, target.posY + 1.0D, target.posZ);
                  entity.world.func_72956_a(entity, this.sound, 0.5F, entity.world.field_73012_v.nextFloat() * 0.1F + 0.9F);
               }
            }
         }
      }
