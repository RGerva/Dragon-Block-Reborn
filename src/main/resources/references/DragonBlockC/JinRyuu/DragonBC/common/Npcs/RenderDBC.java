...
public class RenderDBC extends RendererLivingEntity {
   ...
   protected ResourceLocation func_110775_a(Entity entity) {
      ...
      } else if (entity instanceof EntityDBC) {
         return new ResourceLocation(((EntityDBC)entity).getTexture());
      } else if (entity instanceof EntityJRMC) {
      ...
      } else if (entity instanceof EntityDragon2) {
         return new ResourceLocation(((EntityDragon2)entity).getTexture());
      } else {
         return entity instanceof EntityNamekFrog ? new ResourceLocation(((EntityNamekFrog)entity).getTexture()) : new ResourceLocation("jinryuudragonbc:npcs/" + entity.func_70005_c_().replaceAll("entity.jinryuudragonblockc.", "").replaceAll(".name", "") + ".png");
      }
   }

   protected void func_77033_b(EntityLivingBase par1EntityLivingBase, double par2, double par4, double par6) {
      if (!MinecraftForge.EVENT_BUS.post(new Pre(par1EntityLivingBase, this, par2, par4, par6))) {
         if (this.b) {
            float f = 1.6F;
            float f1 = 0.016666668F * f;
            double d3 = par1EntityLivingBase.func_70068_e(this.field_76990_c.field_78734_h);
            float f2 = par1EntityLivingBase.func_70093_af() ? NAME_TAG_RANGE_SNEAK : NAME_TAG_RANGE;
            if (d3 < (double)(f2 * f2)) {
               String s = par1EntityLivingBase.func_70005_c_();
               if (par1EntityLivingBase.func_70093_af()) {
                  FontRenderer fontrenderer = this.func_76983_a();
                  GL11.glPushMatrix();
                  GL11.glTranslatef((float)par2 + 0.0F, (float)par4 + par1EntityLivingBase.field_70131_O + 0.5F, (float)par6);
         ...
         MinecraftForge.EVENT_BUS.post(new Post(par1EntityLivingBase, this, par2, par4, par6));
      }
   }
}
