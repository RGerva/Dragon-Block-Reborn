...
public class ModStats {
    ...
    public Map<ModStats.Stats, Float> calculateStats(Map<ModAttributes.Attributes, Float> attributes) {
        return calculateStats(attributes, 1F);
    }

    public Map<ModStats.Stats, Float> calculateStats(Map<ModAttributes.Attributes, Float> attributes,float powerRelease) {
        ...
        float regenBody = attributes.get(ModAttributes.Attributes.CON) * Stats.REGEN_BODY.getBaseValue();
        float regenStamina = attributes.get(ModAttributes.Attributes.CON) * Stats.REGEN_STAMINA.getBaseValue();
        ...
        float regenEnergy = attributes.get(ModAttributes.Attributes.SPI) * Stats.REGEN_ENERGY.getBaseValue();
        float mind = attributes.get(ModAttributes.Attributes.MND) * Stats.MIND_VALUE.getBaseValue();
        float training =
                Stats.TRAINING_BONUS.getBaseValue() + (2F * (float)(Math.floor(attributes.get(ModAttributes.Attributes.MND) / 5.0F)) * (powerRelease / 100f));
